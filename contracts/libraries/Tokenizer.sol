...
library Tokenizer {
    ...
    function tokenMaxSupply(uint256 id) external pure returns (uint256) {
        return (id & SUPPLY_MASK) >> METADATA_BIT;
    }

    function tokenType(uint256 id) external pure returns (uint256) {
        return (id & TYPE_MASK) >> (METADATA_BIT + SUPPLY_BIT);
    }

    function tokenMetaData(uint256 id) external pure returns (uint256) {
        return uint256(uint40(id));
    }

    function tokenCreator(uint256 id) external pure returns (address) {
