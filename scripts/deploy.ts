...
dotenv.config()
import { TokenIdGenerator, Governance, Marketplace, NFTFactory, Collectible721, Collectible1155 } from "../typechain"


const TREASURY = process.env.TREASURY || "";
const VERIFIER = process.env.VERIFIER || "";
const SERVICE_FEE = 128;
const CREATOR_FEE_UB = 1000;
...
async function main() {
    ...
    const collectible1155Factory = await ethers.getContractFactory("Collectible1155")
    ...
    const Collectible721Factory = await ethers.getContractFactory("Collectible721")
    ...
    const marketplace = (await upgrades.deployProxy(MarketplaceFactory, [governance.address, SERVICE_FEE, CREATOR_FEE_UB], { initializer: "initialize" })) as Marketplace
    ...
    const nftFactory = (await upgrades.deployProxy(NFTFactoryFactory, [governance.address], { initializer: "initialize" })) as NFTFactory
